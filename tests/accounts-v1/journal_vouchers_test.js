...
assert.match(api,/\$billNo=jvw_text/);
assert.match(api,/\$lineReference=jvw_text/);
assert.match(api,/'billNo'=>\$l\['billNo'\]/);
assert.match(ui,/Send for Approval/);
assert.match(ui,/function printJournalVoucher/);
for (const label of ['ACCOUNT NAME','BILL NO','REF','Prepared By','Checked By','Manager Accounts','Received By']) assert.ok(ui.includes(label), `Printed JV needs ${label}`);
assert.match(ui,/jvwDebitTotal/);
assert.match(ui,/jvwCreditTotal/);
assert.match(ui,/Approve & Post/);
assert.match(ui,/Previous Journal Vouchers/);
assert.match(ui,/qa\('#jvwLines \.jvw-line'\)/, 'Only actual JV rows are bound and submitted; heading is not a journal line');
