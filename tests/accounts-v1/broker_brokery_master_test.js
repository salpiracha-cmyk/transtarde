...
const exportReceipts=read('api/export_receipts.php');
...
assert.match(admin,/"Indentor"/);
assert.match(auth,/function tt_indentor_profile/);
...
assert.match(exportAccounting,/tt_indentor_profile\(\$indentor\)/);
assert.match(exportAccounting,/indentorEstimatedAmount/);
assert.match(exportReceipts,/EXPORT_INDENTOR_COMMISSION_ACCRUAL/);
assert.match(exportReceipts,/er_line\('5610',\$pkrAmount/);
assert.match(exportReceipts,/er_line\('2170',0,\$pkrAmount/);
assert.match(exportReceipts,/Estimated — loaded, awaiting full invoice payment/);
assert.match(exportReceipts,/creditAdviceRate/);
assert.match(chart,/"code": "5600"[\s\S]*"name": "Sales Brokery \/ Commission"/);
assert.match(chart,/"code": "5610"[\s\S]*"name": "Export Indentor Commission"/);
assert.match(chart,/"code": "2170"[\s\S]*"name": "Export Indentor Commission Payables"/);

console.log('Local Broker Brokery and export Indentor commission audit passed.');
