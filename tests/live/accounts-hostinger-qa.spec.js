...
async function backHome(page) {
  await page.getByRole('button', { name: /Accounts Home/i }).click({ force: true });
...
test('authenticated Accounts live smoke: all approved workspaces render without posting data', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Expenses & Overheads/i }).click({ force: true });
  ...
  await page.getByRole('button', { name: /Expenses & Overheads/i }).click({ force: true });
  await page.getByRole('button', { name: /Salaries & Staff/i }).click({ force: true });
  ...
  await page.getByRole('button', { name: /Expenses & Overheads/i }).click({ force: true });
  await page.getByRole('button', { name: /^Donations/i }).click({ force: true });
  ...
  await page.getByRole('button', { name: /Expenses & Overheads/i }).click({ force: true });
  await page.getByRole('button', { name: /Rent & Recurring/i }).click({ force: true });
  await expect(page.getByText('Add Rent Master', { exact: false })).toBeVisible({ timeout: 30_000 });
  await backHome(page);

  await page.getByRole('button', { name: /^Purchases/i }).click({ force: true });
  await page.getByRole('button', { name: /Other Purchase/i }).click({ force: true });
  ...
  await page.getByRole('button', { name: /Journal Voucher/i }).click({ force: true });
  await expect(page.getByRole('heading', { name: 'Journal Voucher' })).toBeVisible({ timeout: 30_000 });
  await backHome(page);

  await page.getByRole('button', { name: /^Reports/i }).click({ force: true });
