...
test('manual Hostinger QA: Export instruction to Mill and container return', async ({ page }, testInfo) => {
  ...
  await expect(contractPreview.locator('.salesContractPageStamp')).toHaveCount(1);
  const contractPageClearances = await contractPreview.locator('.salesContractPhysicalPage').evaluateAll(pages => pages.map(page => {
    ...
    const protectedBottom = pageRect.bottom - (pageRect.height * 32 / 297);
    return body ? protectedBottom - body.getBoundingClientRect().bottom : -1;
  }));
  ...
  await expect.poll(async () => {
    try {
      return await page.evaluate(async ({ contractRef, lotRef, numbers }) => {
      ...
      if (/Execution context was destroyed|navigation/i.test(String(error))) return false;
      throw error;
    }
  }, {
