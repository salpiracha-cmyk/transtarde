...
assert.equal((app.match(/missing\.length\|\|s\.status==='Completed'\?'':'<button class="btn navy" id="completeLot">MARK LOT COMPLETE<\/button>'/g)||[]).length,2,'completion control is rendered only when every completion gate passes and the lot is not already completed');
