...
assert.ok((docs.sales.match(/salesContractPage/g)||[]).length>=2,'Sales Contract pagination must be content driven');assert.match(docs.sales,/PAGE 1 OF /);assert.match(docs.sales,/A\. Packed in new single PP Bags of 25 kgs each/);assert.match(docs.sales,/B\. Packed in new single PP Bags of 10 kgs each/);assert.match(docs.sales,/Free from live insects/);assert.match(docs.sales,/<b>REF<\/b>/);assert.match(docs.sales,/PAYMENT TERMS|PAYMENT/);
