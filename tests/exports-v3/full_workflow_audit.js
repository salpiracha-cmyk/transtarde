...
lot1.customs.fiAllocations=[{fiId:'FI-1',number:'FI-2026-0001',amount:50000,currency:'USD'}];lot1.customs.gdRefs=[{number:'GD-001',date:'2026-09-19'}];lot1.customs.invoiceValue=t.invoiceLines(lot1,contract,true).reduce((sum,line)=>sum+Number(line.amount||0),0);lot1.customs.openAccount=lot1.customs.invoiceValue-50000;lot1.customs.saved=true;lot1.bl={...lot1.bl,blNo:'BL001',onBoardDate:'2026-09-20',vessel:'MV TEST',voyage:'V01',finalFile:'BL001.pdf',finalDocument:{id:'DOC-BL',name:'BL001.pdf',downloadUrl:'api/export_documents.php?id=00000000000000000000000000000001'},finalized:true};lot1.commercial.saved=true;lot1.covering={...lot1.covering,bank:'Meezan Bank',dispatchDate:'2026-09-21',tracking:'DHL123',dispatched:true,frozen:true};
const docs={sales:t.salesContractPrint(contract),invoice:t.commercialInvoiceDoc(lot1,contract,false),packing:t.packingListDoc(lot1,contract,false),phyto:t.phytoInvoiceDoc(lot1,contract),bl:t.blDraftDoc(lot1,contract),cover:t.coveringDoc(lot1,contract),control:t.lcControlDoc(lot1,contract),draft:t.lcDraftDoc(lot1,contract)};
...
assert.match(docs.sales,/A\. 25 KG/);assert.match(docs.sales,/B\. 10 KG/);assert.match(docs.sales,/Free from live insects/);assert.match(docs.invoice,/LC-99881/);assert.match(docs.invoice,/FI-2026-0001/);assert.match(docs.cover,/GD-001/);assert.equal(t.lotStatus(lot1),'Ready to complete');
