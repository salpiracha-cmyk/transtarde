...
assert.equal((docs.sales.match(/salesContractFlowPage/g)||[]).length,1,'Sales Contract must use continuous content flow');assert.match(docs.sales,/contractFlowLayout/);assert.doesNotMatch(docs.sales,/PAGE 1 OF /);assert.match(docs.sales,/A\. Packed in new single PP Bags of 25 kgs each/);assert.match(docs.sales,/B\. Packed in new single PP Bags of 10 kgs each/);assert.match(docs.sales,/Free from live insects/);assert.match(docs.sales,/<b>REF<\/b>/);assert.match(docs.sales,/PAYMENT TERMS|PAYMENT/);
assert.match(docs.customInvoice,/CUSTOMS INVOICE/);assert.match(docs.customInvoice,/BRAND &amp; MARKING/);assert.match(docs.customInvoice,/NUMBER AND KIND OF PACKAGES/);assert.match(docs.customInvoice,/TOTAL VALUE/);assert.match(docs.customInvoice,/PAYMENT BREAKDOWN/);assert.match(docs.customPacking,/CUSTOM PACKING LIST/);assert.match(docs.customPacking,/BRAND &amp; MARKING/);assert.doesNotMatch(docs.customInvoice,/MSCU123456/);assert.doesNotMatch(docs.customPacking,/Container|Seal|MSCU123456/);assert.doesNotMatch(docs.phyto,/MSCU123456/);
assert.match(docs.invoice,/DRAWEE/);assert.match(docs.invoice,/LC-99881/);assert.match(docs.invoice,/FI-2026-0001/);assert.doesNotMatch(docs.invoice,/PAYMENT TERMS|LESS ADVANCE|BALANCE PAYABLE/);assert.match(docs.packing,/MSCU123456-0/);assert.match(docs.packing,/SL001/);assert.match(docs.bl,/MSCU123456-0/);assert.match(docs.bl,/SL001/);assert.match(docs.bl,/Mv Test/i);assert.match(docs.bl,/oceanBlPage/);assert.match(docs.bl,/OCEAN BILL OF LADING/);assert.doesNotMatch(docs.bl,/docLetterhead|docFooterArt|docAutoSign|bagMarking/);assert.match(docs.coo,/CERTIFICATE OF ORIGIN/);assert.match(docs.coo,/PRODUCT OF PAKISTAN/);assert.match(docs.coo,/135\.000 MTONS/);assert.match(docs.cover,/Meezan Bank/i);assert.match(docs.cover,/GD-001/);assert.match(docs.cover,/DHL123/);assert.equal(t.lotStatus(lot1),'Ready to complete');
assert.match(docs.sales,/Thereafter subject to Seller’s re-confirmation\./);
...
const wrapper=fs.readFileSync(__dirname+'/../main/module.php','utf8');assert.match(wrapper,/baseVersion:Number\(queuedBase\.get\(key\)\?\?keyVersions\.get\(key\)\?\?0\)/);assert.match(wrapper,/tt:shared-conflict/);assert.doesNotMatch(wrapper,/Update needs review|ttSyncNotice/);assert.match(wrapper,/tt_shared_commit_queue_v1/);
