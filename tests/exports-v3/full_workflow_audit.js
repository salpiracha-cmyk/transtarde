...
source=source.replace('mount();',`window.__AUDIT__={state,makeShipment,makeLotRecord,blankDocuments,processStatus,lotStatus,fiUsed,actualTotals,plannedTotals,customsPlannedRows,plannedPhysicalRows,millActualsComplete,invoiceLines,commercialInvoiceDoc,packingListDoc,phytoInvoiceDoc,blDraftDoc,coveringDoc,lcDraftDoc,lcControlDoc,tgInternalDoc,salesContractPrint,defaultDocsFor,effectiveTerms,lcSpecificTerms,contractSpecRows,DEFAULT_QUALITY};mount();`);
...
const docs={sales:t.salesContractPrint(contract),customInvoice:t.commercialInvoiceDoc(lot1,contract,true),customPacking:t.packingListDoc(lot1,contract,true),invoice:t.commercialInvoiceDoc(lot1,contract,false),packing:t.packingListDoc(lot1,contract,false),phyto:t.phytoInvoiceDoc(lot1,contract),bl:t.blDraftDoc(lot1,contract),cover:t.coveringDoc(lot1,contract),control:t.lcControlDoc(lot1,contract),draft:t.lcDraftDoc(lot1,contract)};
...
assert.equal((docs.sales.match(/<section class="docPage branded">/g)||[]).length,3,'standard Sales Contract must render exactly three specimen pages');assert.match(docs.sales,/A\. 25 KG/);assert.match(docs.sales,/B\. 10 KG/);assert.match(docs.sales,/Free from live insects/);assert.match(docs.sales,/Contract Reference/);assert.match(docs.sales,/PAYMENT TERMS|PAYMENT/);
assert.match(docs.customInvoice,/CUSTOM INVOICE/);assert.match(docs.customPacking,/CUSTOM PACKING LIST/);assert.match(docs.customPacking,/Brand \/ Packing/);assert.doesNotMatch(docs.customInvoice,/MSCU123456/);assert.doesNotMatch(docs.customPacking,/Container|Seal|MSCU123456/);assert.doesNotMatch(docs.phyto,/MSCU123456/);
assert.match(docs.invoice,/LC-99881/);assert.match(docs.invoice,/FI-2026-0001/);assert.match(docs.packing,/MSCU123456-0/);assert.match(docs.packing,/SL001/);assert.match(docs.bl,/MSCU123456-0/);assert.match(docs.bl,/SL001/);assert.match(docs.bl,/MV TEST/);assert.match(docs.cover,/Meezan Bank/i);assert.match(docs.cover,/GD-001/);assert.match(docs.cover,/DHL123/);assert.equal(t.lotStatus(lot1),'Ready to complete');
...
const single={...contract,packings:[contract.packings[0]],containers:10,qty:270};assert.doesNotMatch(t.salesContractPrint(single),/A\. 25 KG/);

const tgContract={...contract,id:'CT-TG',ref:'TG/NS/02',seller:'TG',packings:[contract.packings[0]]};t.state.contracts.push(tgContract);const tgProcess=t.makeShipment(tgContract);tgProcess.lc={saved:false,documents:[],conditions:[]};const tgLot=t.makeLotRecord(tgProcess,{lotId:'TG-LOT-1',allocations:[{packIndex:0,name:'TTI Rice Mills',type:'TTI',containers:1,weightPer:27}]});tgLot.customs.rate=300;assert.match(t.tgInternalDoc(tgLot,tgContract),/Pakistan-side Value/);assert.doesNotMatch(t.tgInternalDoc(tgLot,tgContract),/NaN|undefined|null/);
