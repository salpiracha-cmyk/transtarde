...
(async()=>{
 ...
 await page.evaluate(()=>{window.TT_MODULE_ACCESS={user:'Document QA',masters:{banks:[{values:['BANK-QA','TTI TRANSTRADE INTERNATIONAL','','TRANSTRADE INTERNATIONAL','QA BANK PAKISTAN','QA Trade Branch, Main Road','Pakistan','USD','QA-ACCOUNT','QA-IBAN-NOT-FOR-PAYMENT','QA-SWIFT','','','Active']},{values:['TG-BANK-QA','TG TRANS GRAINS FOODSTUFF TRADING L.L.C','','TRANS GRAINS FOODSTUFF TRADING L.L.C','QA BANK DUBAI','QA Dubai Branch','United Arab Emirates','USD','QA-TG-ACCOUNT','QA-TG-IBAN','QA-TG-SWIFT','','','Active']}]}};window.TT_SHARED_SYNC={saveNow:()=>Promise.resolve({ok:true,revision:1}),flush:()=>{}};window.confirm=()=>true;window.alert=message=>window.__alert=message;window.print=()=>{window.__prints=(window.__prints||0)+1}});
 ...
 async function render(markup,name,pdf=false){
  ...
  assert.ok(await page.evaluate(()=>{const cover=document.querySelector('#printRoot .bankCoveringPage');return !cover||parseFloat(getComputedStyle(cover).paddingTop)>=57*96/25.4-1}),name+' must reserve the physical-letterhead area');
