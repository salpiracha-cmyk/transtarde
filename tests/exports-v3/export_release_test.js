...
assert.equal((normalContract.match(/salesContractPhysicalPage/g)||[]).length,3,'the representative Sales Contract uses three explicit physical pages');
...
assert.match(normalContract,/Page 1 of 3[\s\S]*Page 2 of 3[\s\S]*Page 3 of 3/,'page numbering is printed in each DATE row');
assert.equal((normalContract.match(/salesContractPageStamp/g)||[]).length,3,'every contract page carries the seller signature and stamp');
