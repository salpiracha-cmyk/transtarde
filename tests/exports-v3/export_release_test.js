...
assert.equal((normalContract.match(/salesContractFlowPage/g)||[]).length,1,'Sales Contract uses one continuous print flow so the browser paginates by available height');
assert.doesNotMatch(normalContract,/salesContractPage/,'fixed page buckets are not used');
assert.match(normalContract,/<tfoot>[\s\S]*contractPrintFooter[\s\S]*contractPageNumber/,'the browser repeats the footer and sequential page number on every printed page');
...
const longContractOutput=t.salesContractPrint(longContract);assert.equal((longContractOutput.match(/salesContractFlowPage/g)||[]).length,1,'long Sales Contract remains one natural print flow');assert.match(longContractOutput,/Long contract term 18/);assert.match(longContractOutput,/Document 12/);
...
for(const label of ['CUSTOMS INVOICE','DRAWEE','VESSEL / VOYAGE','PAYMENT TERMS','NET WEIGHT','GROSS WEIGHT','TARE WEIGHT'])assert.match(customsInvoice,new RegExp(label.replace('/','\\/')));
