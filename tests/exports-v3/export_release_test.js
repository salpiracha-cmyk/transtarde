...
assert.ok((normalContract.match(/salesContractPage/g)||[]).length>=2,'Sales Contract uses deterministic physical pages');
assert.match(normalContract,/PAGE 1 OF \d+/);
...
const longContractOutput=t.salesContractPrint(longContract);assert.ok((longContractOutput.match(/salesContractPage/g)||[]).length>=3,'long Sales Contract paginates into deterministic physical pages');assert.match(longContractOutput,/PAGE 1 OF \d+/);
