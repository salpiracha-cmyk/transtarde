...
s.customs.tcNumber='TC-7788';s.customs.ecNumber='EC-9911';const phytoWithControls=t.phytoInvoiceDoc(s,c);assert.match(phytoWithControls,/TC NUMBER[\s\S]*<strong>TC-7788<\/strong>[\s\S]*EC NUMBER[\s\S]*<strong>EC-9911<\/strong>/);assert.match(phytoWithControls,/CFR Jebel Ali, United Arab Emirates/);assert.match(phytoWithControls,/HS CODE: 1006\.30/);assert.match(phytoWithControls,/BRAND &amp; MARKING/);assert.doesNotMatch(t.commercialInvoiceDoc(s,c,true),/TC-7788|EC-9911/,'TC and EC numbers must not appear outside the Phytosanitary Invoice');
const plannedBagShipment=t.makeShipment(c);plannedBagShipment.kind='lot';plannedBagShipment.containers=2;plannedBagShipment.loadingPlan={physicalContainers:2,allocations:[{packIndex:0,containers:2,weightPer:27}]};plannedBagShipment.millActuals=[{number:'MSCU123456-7',seal:'',bags:0,netKg:27000},{number:'TGHU765432-1',seal:'',bags:0,netKg:27000}];assert.deepEqual(t.actualRows(plannedBagShipment,c).map(row=>row.bags),[1080,1080],'missing Mill bags fall back to Jazib loading instructions');plannedBagShipment.millActuals[0].bags=1075;assert.deepEqual(t.actualRows(plannedBagShipment,c).map(row=>row.bags),[1075,1080],'Mill-entered bags override only that container automatically');
const bagOnlyShipment=t.makeShipment(c);bagOnlyShipment.kind='lot';bagOnlyShipment.containers=2;bagOnlyShipment.loadingPlan={physicalContainers:2,allocations:[{packIndex:0,containers:2,weightPer:27}]};bagOnlyShipment.millActuals=[{number:'MSCU123456-7',bags:1080,netKg:0,brand:'STAR',packing:'25 KG'},{number:'TGHU765432-1',bags:1080,netKg:0,brand:'STAR',packing:'25 KG'}];assert.deepEqual(t.actualRows(bagOnlyShipment,c).map(row=>row.netKg),[27000,27000],'bag-only Mill rows calculate their net weight from actual bags and bag size');
bagOnlyShipment.customs.gdRefs=[];bagOnlyShipment.customs.fiAllocations=[];const cleanedDescription=t.syncBLDynamicDescription(bagOnlyShipment,c,'CUSTOM GOODS TEXT\nTOTAL NET WEIGHT: 216.000 M.TONS / 216,000.00 KGS\nTOTAL GROSS WEIGHT: 216.000 M.TONS / 216,000.00 KGS\nGD NO. / DATE: OLD-GD');assert.match(cleanedDescription,/CUSTOM GOODS TEXT/);assert.match(cleanedDescription,/TOTAL NET WEIGHT: 54\.000 M\.TONS/);assert.doesNotMatch(cleanedDescription,/OLD-GD/);assert.equal(t.fiGdRows({...bagOnlyShipment,customs:{...bagOnlyShipment.customs,gdRefs:[{number:'',date:''}]}}).gd,'','blank GD rows never leak into the B/L');
...
const phytoComparable=phytosanitary.replaceAll('PHYTOSANITARY INVOICE','CUSTOMS INVOICE').replace(/<b class="phytoControlNumber">TC NUMBER<\/b><span class="phytoControlNumber"><strong>.*?<\/strong><\/span><b class="phytoControlNumber">EC NUMBER<\/b><span class="phytoControlNumber"><strong>.*?<\/strong><\/span>/,'');
assert.equal(phytoComparable,customsInvoice,'Phytosanitary must retain the Customs Invoice structure apart from its title and dedicated TC/EC controls');
