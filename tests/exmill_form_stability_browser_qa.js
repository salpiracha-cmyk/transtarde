...
Object.assign(w,{STORE_EXLOAD:'loads',STORE_EXMILL:'sodas',STORE_EXINSTR:'instructions',get:(k,d)=>structuredClone(stores[k]??d),set:(k,v)=>{stores[k]=structuredClone(v)},escHtml:s=>String(s??'').replace(/[&<>"']/g,''),millProductIdentity:(base,stage,type)=>({displayName:[base,type,'Ready Rice'].filter(Boolean).join(' ')}),today:()=> '2026-09-25',containerFormat:v=>v,formatTruckTyping:v=>v,exSelectedMill:'Al-Harmain Rice Mills'});
w.eval(`let exOpenKey='',exSelectedMill='Al-Harmain Rice Mills';${fn('exMillSodas')}${fn('exMillInstructions')}${fn('renderExMillWorkspace')}${fn('toggleExInline')}${fn('renderExLoadForm')}`);
