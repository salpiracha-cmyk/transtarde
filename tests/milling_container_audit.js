...
assert.match(html,/save only the Container Number now/,'Milling can save a valid container number before the remaining loading details');
assert.match(html,/Gate Pass remains locked/,'partial container saves must keep Gate Pass printing locked');
assert.match(html,/function canCompleteShipment[\s\S]{0,700}loadingComplete/,'a shipment cannot complete while any saved container remains partial');
...
console.log('PASS milling container audit: ISO identity, partial-save gate lock, canonical persistence, and failed-save correction are enforced');
