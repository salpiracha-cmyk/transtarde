...
async function updateNotice(known=null){
  const entries=known||await list();document.getElementById('ttOfflineNotice')?.remove();
...
window.fetch=async function(input,init={}){
  ...
  try{const response=await nativeFetch(input,requestInit);await processResponse(entry,response);return response}catch(error){entry.status='offline';entry.error='Server acknowledgement was not received.';entry.updatedAt=new Date().toISOString();await put(entry);await updateNotice();await offerRecovery();throw error;}
};
