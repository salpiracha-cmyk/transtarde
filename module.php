...
(()=>{
  ...
  function exportsToMill(){
    ...
    (root.millSync.newExportBags||[]).forEach((x,index)=>{const line=Number(x.line??x.lineId??index+1)||index+1,lineKey='L'+line,bid=[x.poNo||x.contractRef||'NO-PO',line,x.contractRef||'',x.brand||'',x.size||'',x.orderedTare||''].join('|'),legacy=oldBags.get(bid)||[...oldBags.values()].find(o=>o.poNo===x.poNo&&String(o.poLineKey||'L'+(String(o._ttBridgeId||'').split('|')[1]||'1'))===lineKey&&!o.isMasterBag)||{},common={contractRef:x.contractRef,poNo:x.poNo,supplier:x.supplier||'',mill:x.deliverTo||'TTI Rice Mills',artworkName:x.artworkName||'',artworkData:x.artworkData||'',status:x.status||'Order from Export — Awaiting Receipt'};bags.push({...legacy,...common,id:legacy.id||stableId('bag|'+bid),_ttBridge:'exports',_ttBridgeId:bid,poLineKey:lineKey,isMasterBag:false,brand:x.brand||'UNNAMED BRAND',size:String(x.size||'')+' '+(String(x.unit||'KG').toUpperCase().startsWith('LB')?'lb':'kg'),tare:String(x.orderedTare||0)+' g',ordered:Number(x.totalOrdered||x.requiredBags||0),received:Number(legacy.received||0)});if(x.masterBag?.enabled&&Number(x.masterBag.qty||0)>0){const masterKey=lineKey+'-MASTER',masterBid=bid+'|MASTER',oldMaster=oldBags.get(masterBid)||[...oldBags.values()].find(o=>o.poNo===x.poNo&&o.poLineKey===masterKey)||{};bags.push({...oldMaster,...common,id:oldMaster.id||stableId('bag|'+masterBid),_ttBridge:'exports',_ttBridgeId:masterBid,poLineKey:masterKey,isMasterBag:true,brand:(x.brand||'UNNAMED BRAND')+' — MASTER',size:String(x.masterBag.size||'Master Bag'),tare:String(x.masterBag.tare||0)+' g',ordered:Number(x.masterBag.qty||0),received:Number(oldMaster.received||0)})}});put('tt30bags',bags);
