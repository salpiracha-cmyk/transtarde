...
(()=>{
  ...
  function millToExports(){
    ...
    const add=(shipment,row,location)=>{const ref=String(shipment||''),target=root.shipments.find(s=>String(s.id)===ref||String(s.contractRef)===ref||(s.loading?.lots||[]).some(l=>String(l.lotId)===ref));if(!target||!row?.container)return;target.millActuals=target.millActuals||[];const clean=String(row.container).replace(/[^A-Za-z0-9]/g,'').toUpperCase(),number=/^[A-Z]{4}\d{7}$/.test(clean)?clean.slice(0,10)+'-'+clean.slice(10):String(row.container).toUpperCase();if(target.millActuals.some(a=>String(a.number).replace(/[^A-Za-z0-9]/g,'').toUpperCase()===clean))return;const c=(root.contracts||[]).find(c=>c.ref===target.contractRef)||{},pk=(c.packings||[]).find(p=>String(p.brand||'').toUpperCase()===String(row.brand||'').toUpperCase())||(c.packings||[])[0]||{},bags=Number(row.bags||0),tare=bags*Number(pk.tare||0)/1000+(pk.masterBag?.enabled?Number(pk.masterBag.qty||0)*Number(pk.masterBag.tare||0)/1000:0),net=Number(row.weight??row.kg??0);target.millActuals.push({number,seal:String(row.seal||'').toUpperCase(),packing:`${pk.size||''} ${c.packingUnit||'KG'}`,packSize:Number(pk.size||0),unit:c.packingUnit||'KG',bags,netKg:net,tareKg:tare,grossKg:net+tare,brand:row.brand||pk.brand||'',location,truck:row.truck||'',gatePass:row.gate||'',loadedDate:row.date||'',lotRef:ref,source:'Milling'});root.alerts=root.alerts||[];root.alerts.unshift({id:'AL-'+Date.now(),area:'MILL ACTUALS',contractRef:target.contractRef,kind:'Mill Update',message:`Container ${number} / seal ${row.seal||''} received from Mill`,createdAt:new Date().toISOString(),seen:false});changed=true};
