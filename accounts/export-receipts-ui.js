(() => {
  ...
  function tgBankNotice() {
    if (payerType !== 'TG' || selectedTgPayment) return '';
    const matching = (tgData?.banks || []).filter(bank => String(bank.currency || '').toUpperCase() === currency);
    const messages = [];
    if (!matching.some(bank => bank.settings?.allowPayments)) messages.push(`No TG ${currency} paying account is enabled. Check Active and Allow Payments in Accounts → Banks & Cash; add or amend TG USD/AED accounts in TG Master.`);
    if (tgData && !tgData.rates) messages.push('The TG accounting rate for this date is missing. Enter the bank advice realization rate below; an effective USD → AED rate is also needed in TG Master before a TG payment can be posted.');
    return messages.map(message => `<div class="tter-alert">${esc(message)}</div>`).join('');
  }
  ...
  function calc() {
    ...
    const allocation = [...chosen.values()].reduce((sum,item) => sum + num(item.applied),0), retentionAmount=num(q('#erRetention')?.value), retentionValid=retentionAmount<=received&&(!retentionAmount||!!q('#erRetentionBank')?.value), balanced = Math.abs(allocation - received) <= .01 && Math.abs(bankCredit - bankExpected) <= 2 && retentionValid;
