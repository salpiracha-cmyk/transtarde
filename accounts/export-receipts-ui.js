(() => {
  ...
  let data = null, banks = null, tgData = null, bankId = '', tgBankId = '', payerType = '', payer = '', currency = 'USD', chosen = new Map(), deductions = [], shortfallClass = '', shortfallNote = '';
  ...
  function bankOptions(currencyCode, selected = '') {
    const rows = receiptBanks(currencyCode), preferred = rows.find(row => row.settings?.defaultReceiptAccount)?.id || '';
  ...
  function tgBankOptions() {
    const rows = (tgData?.banks || []).filter(bank => bank.settings?.allowPayments && String(bank.currency || '').toUpperCase() === currency);
    return `<option value="">Choose TG paying account</option>${rows.map(bank => `<option value="${esc(bank.id)}" ${bank.id === tgBankId ? 'selected' : ''}>${esc(bank.bank || bank.title)} · ${esc(bank.title || '')} · ${esc(bank.currency)} ${fmt(bank.balance?.native || 0)}</option>`).join('')}`;
  }
  ...
  function bind() {
    q('[data-er-close]')?.addEventListener('click', () => { ensurePanel().classList.add('hidden'); bankId = ''; payerType = ''; payer = ''; chosen.clear(); deductions = []; });
    q('#erPkrBank')?.addEventListener('change', event => { bankId = event.target.value; payerType = ''; payer = ''; chosen.clear(); render(); });
    qa('[data-payer-type]').forEach(button => { button.onclick = async () => { payerType = button.dataset.payerType; payer = payerType === 'TG' ? 'TG' : ''; chosen.clear(); tgBankId = ''; if (payerType === 'TG' && !tgData) { try { await loadTg(); } catch (error) { payerType = ''; payer = ''; return toast(String(error.message || error),false); } } render(); }; });
    q('#erPayer')?.addEventListener('change', event => { payer = event.target.value; chosen.clear(); render(); });
    q('#erTgBank')?.addEventListener('change', event => { tgBankId = event.target.value; });
    qa('[data-er-item]').forEach(box => { box.onchange = () => { const item = availableItems().find(row => row.key === box.dataset.erItem); if (box.checked && item) chosen.set(item.key,{...item,applied:item.amount}); else chosen.delete(box.dataset.erItem); render(); }; });
    ...
    q('#erCurrency')?.addEventListener('change', event => { currency = event.target.value; tgBankId = ''; chosen.clear(); render(); });
    ...
    ['erForeign','erRate','erGross','erBankCredit','erRetention'].forEach(id => q(`#${id}`)?.addEventListener('input', () => { if (id === 'erForeign') { redistribute(); qa('[data-er-applied]').forEach(input => { input.value = chosen.get(input.dataset.erApplied)?.applied || ''; }); } if (id === 'erForeign' || id === 'erRate') { const gross = num(q('#erForeign')?.value) * num(q('#erRate')?.value); if (q('#erGross') && document.activeElement !== q('#erGross')) q('#erGross').value = gross ? gross.toFixed(2) : ''; } calc(); }));
    q('#erPost')?.addEventListener('click',postReceipt);
  }
  function calc() {
    if (!q('#erForeign')) return;
    captureDeductions(); const expected = selectedExpected(), received = num(q('#erForeign').value), rate = num(q('#erRate')?.value), gross = num(q('#erGross')?.value) || received * rate, retentionPkr = num(q('#erRetention')?.value) * rate;
    const deducted = deductions.reduce((sum,row) => sum + (row.mode === 'DEDUCTED' ? num(row.amount) : 0),0), bankExpected = Math.max(0,gross - retentionPkr - deducted), bankCredit = num(q('#erBankCredit')?.value), shortfall = Math.max(0,expected - received);
    q('#erExpectedForeign').textContent = `${currency} ${fmt(expected)}`; q('#erReceivedForeign').textContent = `${currency} ${fmt(received)}`; q('#erShortfall').textContent = `${currency} ${fmt(shortfall)}`; q('#erExpectedPkr').textContent = `PKR ${fmt(bankExpected)}`;
    shortfallClass = q('#erShortfallClass')?.value || shortfallClass; shortfallNote = q('#erShortfallNote')?.value.trim() || shortfallNote;
    ...
    const allocation = [...chosen.values()].reduce((sum,item) => sum + num(item.applied),0), balanced = Math.abs(allocation - received) <= .01 && Math.abs(bankCredit - bankExpected) <= 2;
    const preview = q('#erAccountingRows'); if (preview) preview.innerHTML = `${bankCredit ? `<div class="tter-accounting-row"><b>Debit</b><span>Selected PKR company bank</span><strong>PKR ${fmt(bankCredit)}</strong></div>` : ''}${retentionPkr ? `<div class="tter-accounting-row"><b>Debit</b><span>Foreign retention bank</span><strong>PKR ${fmt(retentionPkr)}</strong></div>` : ''}${deducted ? `<div class="tter-accounting-row"><b>Debit</b><span>Tax / charges / FED ledgers</span><strong>PKR ${fmt(deducted)}</strong></div>` : ''}${gross ? `<div class="tter-accounting-row"><b>Credit</b><span>${payerType === 'TG' ? 'TG intercompany receivable' : 'Export receivable / customer advance'}</span><strong>PKR ${fmt(gross)}</strong></div>` : '<div class="tter-note">Enter the advice amounts to see the posting.</div>'}<div class="tter-note ${balanced ? 'tter-ok' : 'tter-bad'}">Applied foreign amount: ${currency} ${fmt(allocation)} · bank difference: PKR ${fmt(bankCredit-bankExpected)}</div>`;
    const post = q('#erPost'); if (post) post.disabled = !(received > 0 && rate > 0 && chosen.size && balanced);
  }
  function allocations() { return [...chosen.values()].filter(item => num(item.applied) > 0).map(item => ({targetType:item.targetType,targetId:item.targetId,contractRef:item.contractRef,invoiceRef:item.invoiceRef,customer:item.customer,foreignAmount:num(item.applied)})); }
  ...
  async function postTgSettlements(receipt, classification) {
    ...
    for (const item of chosen.values()) {
      ...
      sequence += 1;
      const payload = {action:'post_payment',csrf:access.csrf,date:q('#erDate')?.value || today(),paymentType:'LIABILITY',sourceLiabilityId:item.mirrorCandidateId,bankAccountId:tgBankId,counterparty:entity(),amountNative:amount,bankChargeNative:0,rate,bankReference:`${receipt.bankAdviceRef}-TG-${sequence}`,rateOverrideNote:'',notes:`Mirrored settlement for Pakistan receipt ${receipt.id}`};
  ...
  async function postReceipt() {
    ...
    try { validateTgSettlement(classification); } catch (error) { return toast(String(error.message || error),false); }
    const body = {action:'post_receipt',csrf:access.csrf,entity:entity(),date:q('#erDate')?.value || today(),bankAdviceRef:q('#erBankRef')?.value.trim() || '',bankAdviceFileRef:'',bankAdvicePaperRef:q('#erFileRef')?.value.trim() || '',remitter:payer,transactionCurrency:currency,foreignAmount:received,realizationRate:num(q('#erRate')?.value),grossPkrEquivalent:num(q('#erGross')?.value),bankAccountId:bankId,pkrBankCredit:num(q('#erBankCredit')?.value),retentionForeignAmount:num(q('#erRetention')?.value),retentionBankAccountId:q('#erRetentionBank')?.value || '',allocations:allocations(),deductions:deductionPayload(),shortfallClassification:classification,shortfallNote:currentShortfallNote};
    ...
    try {
      ...
      await postTgSettlements(result.receipt,classification);
      await postShortfalls(result.receipt,classification,currentShortfallNote,body.bankAdviceFileRef || body.bankAdvicePaperRef);
      toast(`Receipt ${result.receipt.id} posted.`); payerType = ''; payer = ''; chosen.clear(); deductions = []; await load(); render(); window.TT_BANK_ACCOUNTS_UI?.reload?.();
    } catch (error) { toast(String(error.message || error),false); button.disabled = false; button.textContent = 'POST RECEIPT'; }
  }
  async function openForm() {
    if (!['TTI','BRM'].includes(entity())) return toast('Pakistan export receipts are available in TTI / BRM books.',false);
    try {
      q('#ttNativeLaunchers .appCard[data-key="bank"], #homeGrid .appCard[data-key="bank"]')?.click();
      await load(); bankId = receiptBanks('PKR').find(row => row.settings?.defaultReceiptAccount)?.id || ''; ensurePanel(); render(); ensurePanel().scrollIntoView({behavior:'smooth',block:'start'});
