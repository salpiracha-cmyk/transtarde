...
const setFocus=v=>sessionStorage.setItem(key,v==='CORN'?'CORN':'RICE');
...
function switchTo(v){setFocus(v);try{window.TT_RICE_PAYABLE_LINK?.clear?.()}catch{}q('[data-tt-settlement-box]')?.classList.add('hidden');inject();refreshVisible()}
const obs=new MutationObserver(()=>{if(q('#ws-payables'))inject()});obs.observe(document.documentElement,{childList:true,subtree:true});
document.addEventListener('click',e=>{if(e.target.closest('.appCard[data-key="payables"]')){if(!sessionStorage.getItem(key))setFocus('RICE');setTimeout(inject,30)}if(e.target.closest('[data-back]'))q('#ttPayCommodityBar')?.remove()});
