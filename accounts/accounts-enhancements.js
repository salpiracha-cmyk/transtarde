(()=>{
  ...
  function buildEntityLanding(){
    ...
    const power=q('.power');power?.parentElement.insertBefore(change,power);change.onclick=showLanding;qa('[data-tt-entity]').forEach(b=>b.onclick=()=>openEntity(b.dataset.ttEntity));
  }
  ...
  function openEntity(code){const hidden=q(`.entityBtn[data-entity="${code}"]`);if(hidden)hidden.click();else localStorage.setItem('tt_accounts_entity',code);q('#ttEntityLanding').style.display='none';q('#entityHome').style.display='block';scrollTo(0,0)}
