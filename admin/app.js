(() => {
  ...
  function bankMasterFieldsHtml(values = []) {
    ...
    <section class="master-editor-section"><div class="master-editor-heading"><div><h3>Bank details</h3><p>The main register will not expose all banking identifiers.</p></div></div><div class="master-identity-grid">
      ...
      <label>Currency<select id="${masterInputId(7)}" data-master-field-index="7">${["","PKR","USD","AED","EUR","GBP",...(!(["","PKR","USD","AED","EUR","GBP"].includes(values[7])||!values[7])?[values[7]]:[])].map(code=>`<option value="${escapeHtml(code)}" ${code===(values[7]||"")?"selected":""}>${escapeHtml(code||"Select currency")}</option>`).join("")}</select></label>
    </div></section>
