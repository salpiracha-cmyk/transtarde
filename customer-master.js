(()=>{
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='ttCustomerMasterStyle';s.textContent=`
      ...
      .tt-cm-home-button{white-space:nowrap}.tt-cm-master-groups{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-bottom:14px}.tt-cm-master-group{border:1px solid #dbe4ee;border-radius:12px;padding:12px;background:#f8fafc}.tt-cm-master-group b{display:block;color:#17324d;margin-bottom:5px}.tt-cm-master-group span{font-size:12px;color:#64748b;line-height:1.4}
      @media(max-width:720px){.tt-cm-grid,.tt-cm-master-groups{grid-template-columns:1fr}.tt-cm-field.full{grid-column:auto}.tt-cm-table{font-size:13px}.tt-cm-table th:nth-child(3),.tt-cm-table td:nth-child(3){display:none}}
  ...
  function renderManager(body){
    const masters=window.TT_MODULE_ACCESS?.masters||{},count=t=>Array.isArray(masters[t])?masters[t].filter(r=>String(r?.values?.at(-1)||'Active').toLowerCase()!=='inactive').length:0;
    body.innerHTML=`<div class="tt-cm-master-groups"><div class="tt-cm-master-group"><b>CUSTOMER DATA</b><span>Buyers, addresses, notify parties and packing defaults. ${listCache.filter(x=>String(x.status||'Active').toLowerCase()!=='inactive').length} active customer(s).</span></div><div class="tt-cm-master-group"><b>CONTRACT DATA</b><span>Products, document presentation and other terms from shared masters. ${count('products')} product(s), ${count('export_documents')} document rule(s), ${count('export_terms')} term rule(s).</span></div><div class="tt-cm-master-group"><b>SHIPMENT / OPERATIONAL DATA</b><span>Mills, locations, companies and banks remain controlled through Super Admin according to user permissions.</span></div></div><div class="tt-cm-toolbar"><input id="ttCmSearch" placeholder="Search customer, code, country"><button class="tt-cm-btn green" id="ttCmAdd">+ ADD CUSTOMER</button></div><div class="tt-cm-note" style="margin-bottom:10px">Customer data is shared with Export Sales Contracts and Super Admin → Parties. Customers already used in history are archived instead of erased.</div><div id="ttCmRows"></div>`;
