...
function ss_prepare_allocations(array $store,string $entity,array $raw): array {
    ...
    $out=[];$total=0.0;$commodity=0.0;$brokerage=0.0;$logistics=0.0;$withholding=0.0;$brokers=[];$debits=[];$requestUsed=[];
    foreach($raw as $r){
        ...
        $lineWht=round($split['brokerage']*min(100,max(0,(float)($bill['brokerageWhtPercent']??0)))/100,2);$withholding+=$lineWht;
    ...
    return ['rows'=>$out,'total'=>round($total,2),'withholding'=>round($withholding,2),'netPayment'=>round($total-$withholding,2),'commodity'=>round($commodity,2),'brokerage'=>round($brokerage,2),'logistics'=>round($logistics,2),'debits'=>$debits,'broker'=>$unique[0]??''];
}
...
try{
    ...
    try{
        ...
        if($action==='post_supplier_payment'){
            ...
            $store['supplierSettlements'][$id]=array_merge(['id'=>$id,'entity'=>$entity,'date'=>$date,'type'=>'Bank / Cash Supplier Payment','broker'=>$prepared['broker'],'amount'=>$prepared['total'],'withholding'=>$prepared['withholding'],'netPayment'=>$prepared['netPayment'],'allocations'=>$prepared['rows'],'reference'=>$reference,'journalId'=>$journal['id'],'status'=>'Posted','createdAt'=>gmdate('c'),'createdBy'=>(string)($user['full_name']??$user['username']??'Accounts')],$sourceMeta);$result=$store['supplierSettlements'][$id];
        }
