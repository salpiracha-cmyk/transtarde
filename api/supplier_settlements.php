...
function ss_component_capacity(array $store,string $entity,array $bill,array $alloc): array {
    ...
    return ['commodity'=>max(0,round($commodityCap-$prev['commodity'],2)),'brokerage'=>max(0,round($brokerageCap-$prev['brokerage'],2)),'total'=>max(0,round($share-$prev['total'],2))];
}
...
function ss_prepare_allocations(array $store,string $entity,array $raw): array {
    ...
    foreach($raw as $r){
        ...
        $out[]=['billId'=>$billId,'billNo'=>(string)($bill['billNo']??''),'sourceKey'=>$sourceKey,'soda'=>(string)(($bill['sodas'][0]??'')?:''),'broker'=>(string)($bill['broker']??''),'truck'=>(string)($alloc['truck']??''),'pohanch'=>(string)($alloc['pohanch']??''),'dueDate'=>(string)($alloc['dueDate']??''),'amount'=>$amount,'commodityAmount'=>$split['commodity'],'brokerageAmount'=>$split['brokerage']];
    }
...
try{
    ...
    if($_SERVER['REQUEST_METHOD']==='GET'){
        $entity=ss_entity((string)($_GET['entity']??'TTI'));$store=ss_read();$adv=[];
