...
function customer_rows(): array { $m=tt_list_masters();$rows=is_array($m['parties']??null)?$m['parties']:[];$out=[];foreach($rows as $row){$c=customer_row($row);if(preg_match('/\bbuyer\b/i',$c['roles']))$out[]=$c;}usort($out,fn($a,$b)=>strcasecmp($a['name'],$b['name']));return$out; }
...
try{
    ...
    $action=(string)($body['action']??'');$masters=tt_list_masters();$partyRows=is_array($masters['parties']??null)?$masters['parties']:[];
    if($action==='upsert'){
        ...
        $created=false;if($match){$existing=array_values(is_array($match['values']??null)?$match['values']:[]);while(count($existing)<16)$existing[]='';if($mode==='sync'){for($i=0;$i<16;$i++){if($incoming[$i]!==''&&!($i===10&&$existing[$i]!==''))$existing[$i]=$incoming[$i];}if($existing[2]==='')$existing[2]='Export Buyer';if($existing[8]==='')$existing[8]='KG';if($existing[10]==='')$existing[10]='Active';$values=$existing;}else$values=$incoming;tt_update_master('parties',(string)$match['id'],$values);$mid=(string)$match['id'];tt_audit((int)$user['id'],(string)$user['username'],'Updated Export customer master '.$name);}else{$mid=tt_create_master('parties',$incoming);$created=true;tt_audit((int)$user['id'],(string)$user['username'],'Created Export customer master '.$name);}
    ...
    if($action==='delete'){
        ...
        $v=array_values(is_array($match['values']??null)?$match['values']:[]);while(count($v)<16)$v[]='';$used=customer_is_used($match)||!empty($body['used']);if($used){$v[10]='Inactive';tt_update_master('parties',$mid,$v);tt_audit((int)$user['id'],(string)$user['username'],'Archived used Export customer '.$v[0]);customer_respond(['ok'=>true,'archived'=>true,'message'=>'Customer is used in export history and was archived instead of erased.','customers'=>customer_rows()]);}
        tt_delete_master('parties',$mid);tt_audit((int)$user['id'],(string)$user['username'],'Deleted unused Export customer '.$v[0]);customer_respond(['ok'=>true,'deleted'=>true,'message'=>'Unused customer deleted.','customers'=>customer_rows()]);
    }
