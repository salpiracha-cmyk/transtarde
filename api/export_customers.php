...
function customer_identity(string $value): string { return strtolower((string)preg_replace('/[^a-z0-9]+/i','',trim($value))); }
...
try{
    ...
    if($action==='upsert'){
        ...
        $match=null;foreach($partyRows as $row){$v=$row['values']??[];if(($mid!==''&&(string)($row['id']??'')===$mid)||($mid===''&&customer_identity((string)($v[0]??''))===customer_identity($name))||($mid===''&&$code!==''&&strcasecmp((string)($v[1]??''),$code)===0)){$match=$row;break;}}
        $created=false;if($match){$existing=array_values(is_array($match['values']??null)?$match['values']:[]);while(count($existing)<22)$existing[]='';for($i=0;$i<16;$i++){if($mode!=='sync'||($incoming[$i]!==''&&!($i===10&&$existing[$i]!=='')))$existing[$i]=$incoming[$i];}if($existing[2]==='')$existing[2]='Export Buyer';if($existing[8]==='')$existing[8]='KG';if($existing[10]==='')$existing[10]='Active';$values=$existing;tt_update_master('export_customers',(string)$match['id'],$values);$mid=(string)$match['id'];tt_audit((int)$user['id'],(string)$user['username'],'Updated Export customer master '.$name);}else{$mid=tt_create_master('export_customers',$incoming);$created=true;tt_audit((int)$user['id'],(string)$user['username'],'Created Export customer master '.$name);}
