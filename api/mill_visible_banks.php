...
try{
 ...
 foreach((array)($masters['banks']??[]) as $r){if(!is_array($r))continue;$v=array_values((array)($r['values']??[]));while(count($v)<14)$v[]='';$id=(string)($r['id']??'');$entity=mvb_entity((string)$v[1]);if(!in_array($entity,['TTI','BRM'],true))continue;if(trim((string)$v[0])!=='Company Account')continue;if(strtoupper(trim((string)$v[7]))!=='PKR')continue;$s=is_array($settings[$id]??null)?$settings[$id]:[];if(!tt_bank_can_transact($id)||empty($s['visibleToMill']))continue;$raw=preg_replace('/\W+/','',trim((string)($v[8]?:$v[9])))??'';$last5=$raw!==''?substr($raw,-5):'';$bank=trim((string)$v[4])?:trim((string)$v[3]);$display=$bank.($last5!==''?' — •••••'.$last5:'');$rows[]=['id'=>$id,'entity'=>$entity,'display'=>$display,'bankName'=>$bank,'currency'=>'PKR'];}
