...
try{
 $u=tt_require_login();if(!tt_user_can_open_module($u,'Accounts'))ad_out(['ok'=>false,'error'=>'Accounts permission required.'],403);$e=ad_entity((string)($_GET['entity']??''));if(($u['role']??'')!=='Super Admin'&&!tt_user_can_access_entity($u,$e,'View'))ad_out(['ok'=>false,'error'=>'You do not have permission for these company books.'],403);$s=ad_store();
 $bank=[];$local=[];$export=[];
