...
function etc_deductions(array $store,string $entity,string $fy): array {
    $rows=[];$matches=(array)($store['exportTaxCertificateMatches']??[]);
    foreach((array)($store['journals']??[]) as $jid=>$j){
        if(!is_array($j)||($j['status']??'')!=='Posted'||($j['entity']??'')!==$entity)continue;$date=(string)($j['date']??'');if($date===''||etc_fy($date)!==$fy)continue;$meta=is_array($j['meta']??null)?$j['meta']:[];
        $taxRows=is_array($meta['exportTaxDeductions']??null)?$meta['exportTaxDeductions']:[];
        foreach($taxRows as $i=>$t){if(!is_array($t))continue;$code=strtoupper(trim((string)($t['taxCode']??'')));if(!in_array($code,['EXP-WHT-FTR','EXP-AWT-NTR'],true))continue;$amount=round((float)($t['amount']??0),2);if($amount<=0)continue;$key=$jid.'|'.(string)$i;$rows[]=['key'=>$key,'journalId'=>$jid,'date'=>$date,'reference'=>(string)($j['reference']??''),'taxCode'=>$code,'taxSection'=>(string)($t['taxSection']??''),'bankAccountId'=>(string)($t['bankAccountId']??$meta['bankAccountId']??''),'bankName'=>(string)($t['bankName']??$meta['bankName']??''),'amount'=>$amount,'foreignCurrency'=>(string)($meta['transactionCurrency']??''),'foreignAmount'=>(float)($meta['transactionAmount']??0),'fiRefs'=>(array)($meta['fiRefs']??[]),'invoiceRefs'=>(array)($meta['invoiceRefs']??[]),'certificateId'=>(string)($matches[$key]??'')];}
    }
...
function etc_payload(array $store,string $entity,string $fy): array {
    ...
    return ['ok'=>true,'entity'=>$entity,'financialYear'=>$fy,'editable'=>true,'deductions'=>$ded,'certificates'=>$certs,'summary'=>['receiptTax'=>$totalDed,'certificateTotal'=>$totalCert,'matchedTax'=>$matched,'unmatchedReceiptTax'=>round($totalDed-$matched,2),'certificateDifference'=>round($totalCert-$matched,2)]];
}
...
try{
    ...
    try{
        ...
        if($action==='save_certificate'){
            ...
            $taxCode=strtoupper(trim((string)($body['taxCode']??'')));if(!in_array($taxCode,['EXP-WHT-FTR','EXP-AWT-NTR'],true))etc_respond(['ok'=>false,'error'=>'Select Withholding Tax (FTR historical) or Advance Withholding Tax (NTR).'],422);
            ...
            $keys=is_array($body['deductionKeys']??null)?$body['deductionKeys']:[];$available=array_column(etc_deductions($store,$entity,$fy),null,'key');foreach($keys as $key){$key=(string)$key;if(!isset($available[$key]))continue;if(($available[$key]['taxCode']??'')!==($cert['taxCode']??''))etc_respond(['ok'=>false,'error'=>'A selected deduction uses a different tax type from the certificate.'],422);if(($cert['bankAccountId']??'')!==''&&($available[$key]['bankAccountId']??'')!==''&&($available[$key]['bankAccountId']??'')!==($cert['bankAccountId']??''))etc_respond(['ok'=>false,'error'=>'A selected deduction belongs to a different bank account.'],422);$store['exportTaxCertificateMatches'][$key]=$certificateId;}
