...
function er_reverse_receipt_store(array &$store, string $receiptId, string $entity, string $date, string $reason, array $user): array {
    ...
    if(isset($store['tgBankTransactions'])&&is_array($store['tgBankTransactions']))foreach($store['tgBankTransactions'] as &$tx)if(is_array($tx)&&in_array((string)($tx['journalId']??''),$ids,true)){$tx['status']='Reversed for Amendment';$tx['reversalJournalId']=$reversed[$tx['journalId']];}unset($tx);
    if(isset($store['exportCandidates'])&&is_array($store['exportCandidates']))foreach($store['exportCandidates'] as &$candidate){if(!is_array($candidate)||!is_array($candidate['meta']??null))continue;$meta=&$candidate['meta'];if(in_array((string)($meta['indentorPayableJournalId']??''),(array)($receipt['indentorPayableJournalIds']??[]),true)&&($meta['indentorFirmReceiptId']??'')===$receiptId){foreach(['indentorPayableJournalId','indentorFirmNativeAmount','indentorFirmPkrAmount','indentorCreditAdviceRate','indentorFirmReceiptId'] as $key)unset($meta[$key]);$meta['indentorPayableStatus']='Estimated — awaiting full invoice payment';}unset($meta);}unset($candidate);
