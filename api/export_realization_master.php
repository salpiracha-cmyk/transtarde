...
const TT_EXPORT_REALIZATION_MASTER_FIELDS = 14;
...
function erm_defaults(): array {
    return [
        ['Export Withholding Tax — FTR (Historical)','EXP-WHT-FTR','Income Tax','FTR — historical','Export proceeds / realization as applicable','','','','Final / source tax expense — legacy FTR treatment','7200','Yes','TTI; BRM','Historical','Historical FTR row. Preserve old treatment and certificates. Do not silently use for current NTR receipts. Capture the exact tax section shown by the bank/certificate on each receipt.'],
        ['Advance Withholding Tax on Export Proceeds','EXP-AWT-NTR','Income Tax','NTR — current','Export proceeds / realization as applicable','','','','Advance income tax / tax deducted at source recoverable','1260','Yes','TTI; BRM','Active','Current NTR treatment as instructed by management. Rate is effective-dated and must be maintained from the applicable Finance Act / auditor advice; actual bank deduction remains authoritative for posting and certificate reconciliation.'],
        ['Export Development Surcharge (EDS)','EXP-EDS','Surcharge','Any','Export realization / prescribed base','0 / currently not charged','','','Export development surcharge expense','6830','No / bank advice evidence','TTI; BRM','Inactive / currently not charged','Keep the row because the surcharge can change with annual budget policy. Activate a new effective-dated row if it becomes applicable again.'],
        ['Export FDBC / Collection / Negotiation Fee','EXP-BANK-FDBC','Bank Fee','Any','Actual bank advice','Actual','','','Export realization / bank charges expense','6810','No','TTI; BRM','Active','Use for export collection, FDBC, negotiation or similar bank fees where separately identified.'],
        ['Export Service / Handling Charges','EXP-BANK-SVC','Bank Fee','Any','Actual bank advice','Actual','','','Export realization / bank charges expense','6810','No','TTI; BRM','Active','Bank service, handling, processing or realization-related charges.'],
        ...
        ['Courier / Document Handling on Export Realization','EXP-COURIER','Document / Courier Charge','Any','Actual bank advice','Actual','','','Bank / export document charge expense','6810','No','TTI; BRM','Active','Use where courier/document handling is deducted or separately charged by the bank.'],
        ['Other Export Realization Charge','EXP-OTHER','Other','Any','Actual bank advice','Actual','','','Bank & finance charges / approved mapped account','6800','No','TTI; BRM','Active','Fallback only when the bank advice contains a genuine charge not covered by another active master row. Accounts should add a dedicated row if the charge recurs.']
    ];
}
...
function erm_payload(array $user): array {
    ...
    return ['ok'=>true,'type'=>TT_EXPORT_REALIZATION_MASTER_TYPE,'editable'=>erm_can_edit($user),'rows'=>$rows,'fields'=>[
        'Charge / tax name','Code','Category','Tax regime / context','Calculation base','Rate % / formula','Effective from','Effective to','Accounting treatment','GL account code','Bank certificate required','Applies to entities','Status','Notes'
    ]];
}
