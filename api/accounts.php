...
function accounts_valid_date(string $date): string {
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) accounts_respond(['ok'=>false,'error'=>'Valid accounting date required.'], 422);
    return $date;
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        accounts_respond([
            ...
            'reminders'=>$store['reminders'],'masters'=>$store['masters'],'accountingMaster'=>$master,'serverNow'=>gmdate('c')
        ]);
    }
