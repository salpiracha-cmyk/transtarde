...
function pp_date(string $v,string $label='Date'): string {
    if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$v))pp_respond(['ok'=>false,'error'=>$label.' is invalid.'],422);
    return $v;
}
...
try{
    ...
    $entity=pp_entity((string)($body['entity']??''));$sourceKey=trim((string)($body['sourceKey']??''));if($sourceKey==='')pp_respond(['ok'=>false,'error'=>'Truck / payable reference is required.'],422);
