...
$entities=array_values(array_filter(['TTI','BRM'],static fn($e)=>tt_inv_can_report($user,$e)));
...
try{$values=tt_inv_read_values();$report=tt_inv_statement($values,$entity);$rate=tt_inv_rate($values,$entity,$today);}
catch(Throwable $error){error_log('Stock reconciliation: '.$error->getMessage());http_response_code(503);exit('The reconciliation report could not be read. No figures have been assumed.');}
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Stock Reconciliation — <?=$escape($entity)?></title>
<style>body{margin:0;background:#f3f6f5;color:#192c27;font:15px/1.5 Arial,sans-serif}header{padding:16px 24px;background:#155d47;color:#fff;display:flex;gap:20px;align-items:center}header a{color:#fff}main{max-width:1250px;margin:24px auto;padding:0 18px}h1{font-size:25px}h2{font-size:20px;margin-top:28px}.summary{display:flex;gap:14px;flex-wrap:wrap}.card{background:#fff;border:1px solid #d8e3de;border-radius:10px;padding:16px;flex:1;min-width:210px}.card strong{display:block;font-size:24px}.note{color:#4c6259}.table{overflow:auto;background:#fff;border:1px solid #d8e3de;border-radius:10px}table{border-collapse:collapse;width:100%;font-size:13px}th,td{padding:10px;border-bottom:1px solid #e1e8e5;text-align:left;vertical-align:top}th{background:#edf4f0}select,button{font:inherit;padding:7px;border:1px solid #9fb8ac;border-radius:6px;background:#fff}form{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.fixed{font-weight:bold}.warning{color:#8a311d}.empty{padding:15px;color:#52645c}@media print{header a,form{display:none}body{background:#fff}.card,table{break-inside:avoid}}</style></head><body>
<header><strong>TRANSTRADE</strong><span>Accounts / Directors</span><a href="/<?=$origin?>/index.php">Return to <?=$escape(ucfirst($origin))?></a></header><main>
<h1>Ghati &amp; Stock Reconciliation</h1>
<form method="get"><input type="hidden" name="origin" value="<?=$escape($origin)?>"><label>Company <select name="entity"><?php foreach($entities as$e):?><option <?=$e===$entity?'selected':''?>><?=$escape($e)?></option><?php endforeach;?></select></label><button type="submit">Refresh report</button><span><?=$escape($date($today))?></span></form>
<p class="note">Informational reconciliation, not physical inventory. Gains first offset Ghati; unused gain carries forward. Actual loading is not blocked while current-shift production is pending. Figures below do not create a purchase or a journal.</p>
